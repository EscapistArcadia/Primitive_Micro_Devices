...
loop_start:
    add     x6, x6, x5      # Add current counter value to accumulator
    addi    x5, x5, -1      # Decrement loop counter

    # Test BEQ: If x5 equals 10, branch to branch_ten to add bonus
    li      x7, 10
    beq     x5, x7, branch_ten

    # Test BNE: If x5 is NOT equal to 5, branch to skip_five
    li      x8, 5
    bne     x5, x8, skip_five

    # If x5 equals 5 (i.e. bne did not take), add extra bonus
    addi    x6, x6, 50
    nop
    nop
    nop
...
skip_five:
    # Test BLT: If x5 is less than 3, exit the loop
    li      x9, 3
    blt     x5, x9, exit_loop

    # (Optional) Test BGE: If x5 is >= 3, continue loop.
    # This bge is redundant here, but it shows the instruction usage.
    bge     x5, x9, continue_loop

continue_loop:
    # Use JAL to jump unconditionally back to loop_start
    jal     x0, loop_start

branch_ten:
    # When x5 equals 10, add an extra bonus to the accumulator
    addi    x6, x6, 100
    # Then jump to skip_five to continue processing
    jal     x0, skip_five

exit_loop:
    # Use JALR as a final jump (simulate a function return or exit)
    # For example, here we load a dummy exit code in x1 (0) and jump via jalr.
    la      x1, done
    jalr    x0, x1, 0
    nop
    nop
